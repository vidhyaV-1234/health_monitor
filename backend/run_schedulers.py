...
def run_all_schedulers():
    ...
    logger.info("  📅 Calendar fetch & backup: 10:55 PM (22:55) - TESTING")
    logger.info("  📍 Location analysis & summary: 10:55 PM (22:55) - TESTING")
    logger.info("  🔔 Mood notifications: 10:55 PM (22:55) - TESTING")
    logger.info("  🎯 Activity recommendations: 10:55 PM (22:55) - TESTING")
    logger.info("="*70)
    logger.info(f"Current time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    
    # Schedule ALL tasks at 10:55 PM (for testing)
    schedule.every().day.at("22:55").do(fetch_calendar_for_all_users)
    schedule.every().day.at("22:55").do(analyze_locations_for_all_users)
    schedule.every().day.at("22:55").do(send_mood_notifications)
    schedule.every().day.at("22:55").do(generate_activity_recommendations)
    
    # Run immediately for testing (if current time is close to scheduled time)
    current_hour = datetime.now().hour
    current_minute = datetime.now().minute
    
    # If it's close to 10:55 PM (within 10 minutes), run all tasks immediately
    if current_hour == 22 and 45 <= current_minute <= 59:
        logger.info("⏰ Current time is close to 10:55 PM, running all tasks immediately...\n")
        fetch_calendar_for_all_users()
        analyze_locations_for_all_users()
        send_mood_notifications()
        generate_activity_recommendations()
    elif current_hour == 22 and current_minute >= 45:
        logger.info("⏰ Current time is close to 10:55 PM, running all tasks immediately...\n")
        fetch_calendar_for_all_users()
        analyze_locations_for_all_users()
        send_mood_notifications()
...
if __name__ == "__main__":
    ...
    if choice == "1":
        ...
        print("  - Calendar fetch & backup: 10:55 PM (22:55) - TESTING")
        print("  - Location analysis & summary: 10:55 PM (22:55) - TESTING")
        print("  - Mood notifications: 10:55 PM (22:55) - TESTING")
        print("  - Activity recommendations: 10:55 PM (22:55) - TESTING")
